...
sql_client_cmd = os.environ['SQL_CLIENT_CMD'] if  'SQL_CLIENT_CMD' in os.environ.keys() else "/opt/mssql-tools/bin/sqlcmd"
random_seed = int(os.environ['RANDOM_SEED']) if  'RANDOM_SEED' in os.environ.keys() else 0
connstr = f"{sql_client_cmd} -S tcp:{sql_client_host},{sql_client_port} -U {sql_client_user} -P {sql_client_pass} "
